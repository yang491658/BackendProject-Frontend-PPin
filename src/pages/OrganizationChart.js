...
const OrganizationChart = () => {
  ...
  const getSubordinates = (managerId) => {
    return employees.filter((employee) => {
      ...
      return (
        (managerId === 201 && employee.companyId === 202) ||
  ...
  const renderOrganizationChart = () => {
    ...
    const firstRowEmployees = employees.filter((employee) =>
      [201, 301, 401, 501, 601].includes(employee.companyId)
    );
            ...
            <div key={firstRowEmployee.companyId} className="department-column">
              ...
              <div className="second-row">
                {getSubordinates(firstRowEmployee.companyId).length > 0 ? (
                  getSubordinates(firstRowEmployee.companyId).map(
                    (secondRowEmployee) => (
                      <div
                        key={secondRowEmployee.companyId}
                        className="subordinate"
                      >
                        {renderEmployeeNode(secondRowEmployee)} {/* 팀장 */}
                        <div className="third-row">
                          {getSubordinates(secondRowEmployee.companyId).map(
                            (thirdRowEmployee) => (
                              <div key={thirdRowEmployee.companyId}>
                                {renderEmployeeNode(thirdRowEmployee)}{" "}
                                {/* 팀원 */}
                                <div className="fourth-row">
                                  {/* 304, 404, 504, 604 표시 */}
                                  {getSubordinates(
                                    thirdRowEmployee.companyId
                                  ).map((fourthRowEmployee) => (
                                    <div key={fourthRowEmployee.companyId}>
                                      {renderEmployeeNode(fourthRowEmployee)}{" "}
                                      {/* 4번째 팀원 */}
                                    </div>
                                  ))}
                                </div>
